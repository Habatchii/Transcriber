...
"""Transcriber support for codeintel.
...
lang = "Transcriber"
log = logging.getLogger("codeintel.transcriber")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Transcriber-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "transcribe,",
...
# lexers. You must define a Lexer class for lang Transcriber. If Komodo's
# scintilla lexer for Transcriber is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class TranscriberLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Transcriber is one of Komodo's existing C++ lexers
...
#   class TranscriberLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_TRANSCRIBER)
#           self._keyword_lists = [
...
# examples of usage.
class TranscriberLangIntel(LangIntel):
    lang = lang
...
#
class TranscriberBuffer(UDLBuffer):
    ...
    #css_lang = "CSS"
    #csl_lang = "JavaScript"
    ssl_lang = "Transcriber"
    #tpl_lang = "Transcriber"
...
# Transcriber content and returns a description of the code in that file.
# See "cile_transcriber.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Transcriber is
...
class TranscriberCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_transcriber
        return cile_transcriber.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=TranscriberLexer(),
        buf_class=TranscriberBuffer,
        langintel_class=TranscriberLangIntel,
        import_handler_class=None,
        cile_driver_class=TranscriberCILEDriver,
