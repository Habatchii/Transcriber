...
"""A Code Intelligence Language Engine for the Transcriber language.
...
Module Usage:
    from cile_transcriber import scan
    mtime = os.stat("bar.transcriber")[stat.ST_MTIME]
    content = open("bar.transcriber", "r").read()
    scan(content, "bar.transcriber", mtime=mtime)
"""
...
class TranscriberCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.transcriber")
...
def scan_buf(buf, mtime=None, lang="Transcriber"):
    """Scan the given TranscriberBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {TranscriberBuffer} is the Transcriber buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Transcriber CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Transcriber content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Transcriber <example-Transcriber-file>
    ...
    # Dev Note:
    # This is where you process the Transcriber content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Transcriber-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Transcriber content, you may
    #   want to use it.
